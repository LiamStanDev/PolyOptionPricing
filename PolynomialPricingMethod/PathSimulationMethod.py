            ...
            if over_jump_ind.size > 0:
                for j in over_jump_ind:
                    ...
                    for k in range(jumps[j]):
                        ...
                        lnYt_over_jump = up_rv_over_jump * exp_rv_over_jump[0] + (1 - up_rv_over_jump) * (-exp_rv_over_jump[1])
                        J[j] += lnYt_over_jump
...
class SVJJByMC:
    ...
    def __init__(self, S0, r, d, T, sigma, intensity, sigma_v, corr, k_v, v_bar, mu_v, mu_y, sigma_y, corr_J, poly_coeff,
                 N_line=int(1e6), n=252, N_repeat=20):
        self.S0 = S0
        self.r = r
        self.d = d
...
class NIGByMC:
    ...
    def Xt_cf(self, u):
        return exp(self.delta * self.T * (
                pow(self.alpha ** 2 - self.beta ** 2, 0.5) - pow(self.alpha ** 2 - (self.beta + 1j * u) ** 2, 0.5)))

    def getValue(self, repeat):

        mu = self.r - log(self.Xt_cf(-1j)) / self.T
        Xt = stats.norminvgauss.rvs(a=self.alpha * self.delta * self.T, b=self.beta * self.delta * self.T,
                                    scale=self.delta * self.T, size=self.N_line)

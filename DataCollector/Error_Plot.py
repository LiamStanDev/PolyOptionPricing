...
def Call():
    ...
    ref_val_close_form = {"GBM": BSMCloseForm(S0=S0, r=r, sigma=sigma, T=T, K=100).getValue(),
                            "Heston": 6.8816576853411586256470400257967412471771240234375,
                            "MJD": MertonCloseForm(S0=S0, T=T, r=r, sigma=sigma, K=100, jump_intensity=jump_intensity, jump_mean=jump_mean, jump_var=jump_var).getValue(5000),
                            "KJD": None,
                            "SVJ": None,
                            "VG": None,
                            "NIG": None,
    ...
    for process_name in processes.keys():
        ...
        densityRecover = DensityRecover(S0=S0, process_cf=processes[process_name], poly_coeff=poly_coeff, positive_interval=positive_interval, error_acceptance=1e-15)
        ...
        if ref_val is None:
            ref_val = PolyByCosMethod(S0=S0, T=T, r=r, sigma=sigma, process_cf=processes[process_name], poly_coeff=poly_coeff,

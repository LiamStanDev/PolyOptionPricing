...
positive_interval = [0, 1.363962, 10.620047, 25.599102, 38.481405, inf] # positive region between roots and boundaries
...
lower_limit, upper_limit, best_positive_interval[0], best_positive_interval[-1] = DensityRecover(S0=S0, process_cf=process_cf,
                                                                                                 poly_coeff=poly_coeff,
                                                                                                 positive_interval=positive_interval).getIntegralRangeAndInterval()
...
for N in [64, 128, 256, 512, 1024]:
    ...
    result, spending_time= timeit(polynomial.getValue) # print the total spending time

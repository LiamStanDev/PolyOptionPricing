...
class BSMCloseForm:
    ...
    def getValue(self):
        ...
        BS_C = self.S0 * stats.norm.cdf(d1, 0.0, 1.0) - self.K * np.exp(
            -self.r * self.T
...
class MertonCloseForm:
    ...
    def getValue(self, iterator_time):
        ...
        r_n = (
            lambda n: self.r
            + n * (self.jump_mean + 0.5 * self.jump_var) / self.T
            - self.jump_intensity * k
        )
        lambda_start = self.jump_intensity * (k + 1)
        d1_n = lambda n: (log(self.S0 / self.K) + (r_n(n) + var_n(n) / 2) * self.T) / (
            sqrt(var_n(n) * self.T)
        )
        d2_n = lambda n: (log(self.S0 / self.K) + (r_n(n) - var_n(n) / 2) * self.T) / (
            sqrt(var_n(n) * self.T)
        )
        Merton_C = 0
        for i in range(iterator_time):
            Merton_C += (
                self.S0 * stats.norm.cdf(d1_n(i), 0.0, 1.0)
            ) * stats.poisson.pmf(i, lambda_start * self.T) - self.K * exp(
                -self.r * self.T
            ) * stats.norm.cdf(
                d2_n(i), 0.0, 1.0
            ) * stats.poisson.pmf(
                i, self.jump_intensity * self.T
            )

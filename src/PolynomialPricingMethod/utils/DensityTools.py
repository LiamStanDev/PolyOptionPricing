...
class DensityRecover:
    ...
    def __init__(
        ...
        self.N = int(N)
        self.step = step
        self.assume_true_a = log(
            assume_true_a
        )  # use for determining true density (benchmark)
        ...
        if len(positive_interval) != 0:
            self.ini_guess_a = (
                log(self.positive_interval[0])
                if self.positive_interval[0] != 0
                else log(self.positive_interval[1])
            )
            self.ini_guess_b = (
                log(self.positive_interval[-1])
                if log(self.positive_interval[-1]) != inf
                else log(self.positive_interval[-2])
            )

            # 確認a是否正確
            if self.positive_interval[0] == 0:
                if (
                    self.ini_guess_a == self.ini_guess_b
                ):  # 若起始a與起始b相同，會產生divided zero error
                    self.ini_guess_a = log(
                        self.positive_interval[1] - 1
                    )  # 因為為0的關係所以將a向左移動10元股價
            ...
            if self.positive_interval[-1] == inf:
                if (
                    ...
                    self.ini_guess_b = log(
                        self.positive_interval[-2] + 1
                    )  # 因為為inf的關係所以將b向右移動10元股價
    ...
    def _calFkCoeff(self):
        ...
        self.Fk_Norm_ls = np.array(
            list(map(self._Fk, k, np.full(self.N, True)))
        )  # 用 k>=0 做一個全部都是true的array
    ...
    def _Fk(self, k, isNorm):
        ...
        if isNorm:
            try:
                inner = self.process.getCFValue(
                    k * pi / (self.assume_true_b - self.assume_true_a)
                ) * exp(
                    -i
                    * k
                    * pi
                    * self.assume_true_a
                    / (self.assume_true_b - self.assume_true_a)
                )
                ...
                if k == 0:
                    density += (
                        ...
                        * cos(
                            k
                            * pi
                            * (x - self.assume_true_a)
                            / (self.assume_true_b - self.assume_true_a)
                        )
                    )
                else:
                    density += self.Fk_Norm_ls[k] * cos(
                        k
                        * pi
                        * (x - self.assume_true_a)
                        / (self.assume_true_b - self.assume_true_a)
    ...
    def _FindBestDensityRange(self):
        ...
        if self.debug:
            count = 0
        while (
            density_low > self.error_acceptance
            or density_high > self.error_acceptance
            or density_low_minus > density_low
            or density_high_plus > density_high
        ):
    ...
    def getIntegralRangeAndInterval(self):
        ...
        if self.positive_interval[0] == 0 and self.positive_interval[-1] == inf:
            ...
            return (
                exp(best_interval_low),
                exp(best_interval_high),
                exp(best_interval_low),
                exp(best_interval_high),
            )
        ...
        # 右側尋找
            ...
            return (
                exp(best_integral_low),
                exp(best_interval_high),
                exp(self.ini_guess_a),
                exp(best_interval_high),
            )
        ...
        # 左側尋找
            ...
            return (
                exp(best_interval_low),
                exp(best_integral_high),
                exp(best_interval_low),
                exp(self.ini_guess_b),
            )
        ...
        # 不用尋找
            ...
            best_integral_low = (
                best_integral_low
                if best_integral_low < self.ini_guess_a
                else self.ini_guess_a
            )
            best_integral_high = (
                best_integral_high
                if best_integral_high > self.ini_guess_b
                else self.ini_guess_b
            )
            print("no searching....")
    ...
    def poltDensity(self, plot_save=False, file_name_prefix=""):
        ...
        if plot_save:
            plt.savefig(
                f"{self.process.__class__.__name__}_{file_name_prefix}_density.jpg"
            )
        plt.show()
    ...
    def plotFittingDetail(self, plot_save=False, file_name_prefix=""):
        ...
        sns.scatterplot(
            x=np.arange(len(self.right_density_plot)), y=self.right_density_plot
        )
        if plot_save:
        ...
        sns.scatterplot(
            x=np.arange(len(self.left_density_plot)), y=self.left_density_plot
        )
        if plot_save:

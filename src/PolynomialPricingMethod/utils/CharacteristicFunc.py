...
class GBM(CharacteristicFunction):
    ...
    def getCFValue(self, u):
        cf_value = exp(
            (self.r - 0.5 * self.sigma**2) * self.T * i * u
...
class Heston(CharacteristicFunction):
    ...
    def getCFValue(self, u):
        alpha = (
            self.mean_reversion_speed - i * self.corr * self.std_of_var_process * u
        ) ** 2
        beta = (u**2 + i * u) * self.std_of_var_process**2
        b = sqrt(alpha + beta)
        a_minus = (
            self.mean_reversion_speed - i * self.corr * self.std_of_var_process * u - b
        )
        a_plus = (
            self.mean_reversion_speed - i * self.corr * self.std_of_var_process * u + b
        )
        ...
        C = (
            ...
            * self.long_term_var_mean
            / pow(self.std_of_var_process, 2)
...
class MJD(CharacteristicFunction):
    ...
    def getCFValue(self, u):
        ...
        cf_value = exp(
            i * u * (mu - pow(self.sigma, 2) / 2) * self.T
            - 0.5 * pow(u * self.sigma, 2) * self.T
            - self.jump_intensity * self.T * (1 - lnY_cf)
        )
        return cf_value
...
class KJD(CharacteristicFunction):
    ...
    def getCFValue(self, u):
        ...
        mu = self.r - self.jump_intensity * k
        lnY_cf = self.p * self.eta1 / (self.eta1 - i * u) + (1 - self.p) * self.eta2 / (
            self.eta2 + i * u
        )
        cf_value = exp(
            i * u * (mu - pow(self.sigma, 2) / 2) * self.T
...
class VG(CharacteristicFunction):
    ...
    def Xt_cf(self, u):
        return pow(
            1
            - i * u * self.gamma_mean * self.gamma_var
    ...
    def Xt_cf(self, u):
        return exp(
            ...
            * (
                ...
                - pow(self.alpha**2 - (self.beta + i * u) ** 2, 0.5)
            )
        )
...
class SVJJ(CharacteristicFunction):
    ...
    def _getCFValue(self, u):
        ...
        beta = (
            -a
            * (1 - exp(-gamma * self.T))
            / (2 * gamma - (gamma + b) * (1 - exp(-gamma * self.T)))
        )

        alpha_0 = (
            -self.r * self.T
            + (self.r - self.d) * i * u * self.T
            - self.k_v
            * self.v_bar
            * (
                (gamma + b) / power(self.sigma_v, 2) * self.T
                + 2
                / power(self.sigma_v, 2)
                * log(1 - (gamma + b) / (2 * gamma) * (1 - exp(-gamma * self.T)))
            )
        )
        c = 1 - self.corr_J * self.mu_v * i * u

        d = (gamma - b) / (
            (gamma - b) * c + self.mu_v * a
        ) * self.T - 2 * self.mu_v * a / (
            power(gamma * c, 2) - power(b * c - self.mu_v * a, 2)
        ) * log(
            1
            - ((gamma + b) * c - self.mu_v * a)
            / (2 * gamma * c)
        ...
        alpha = (
            alpha_0
            - self.intensity * self.T * (1 + mu_bar * i * u)
            + self.intensity * f
        )
...
class SVCDJ(CharacteristicFunction):
    ...
    def _getCFValue(self, u):
        e = sqrt(
            (i * u * self.sigma_y * self.corr - self.k_y) ** 2
            - i * u * (i * u - 1) * self.sigma_y**2
        )
        b = e + i * u * self.sigma_y * self.corr - self.k_y
        q = i * u * (i * u - 1) * self.theta_y + b * (
            1 - self.theta_y * i * u * self.mu_xy
        )
        ...
        B = (
            ...
            * (1 - exp(-e * self.T))
            / (
                2 * e
                - (e + i * u * self.sigma_y * self.corr - self.k_y)
                * (1 - exp(-e * self.T))
            )
        )
        ...
        A -= (
            self.Y_bar
            / power(self.sigma_y, 2)
            * (
                (e + i * u * self.sigma_y * self.corr - self.k_y) * self.T
                + 2
                * log(
                    1
                    - (e + i * u * self.sigma_y * self.corr - self.k_y)
        ...
        A -= (
            ...
            * self.intensity
            * (
                exp(self.mu_0 + 0.5 * self.sigma_xy**2)
                / (1 - self.theta_y * self.mu_xy)
                - 1
            )
            * self.T
            + self.intensity * self.T
        )
        A += (
            self.intensity
            * (2 * e - b)
            * exp(i * u * self.mu_0 - 0.5 * u**2 * self.sigma_xy**2)
            * self.T
            / p
        )
        A += (
